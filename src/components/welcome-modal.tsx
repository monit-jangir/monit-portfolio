...
export default function WelcomeModal({ trigger }: WelcomeModalProps) {
  ...
  const defaultTrigger = (
    ...
    >
      <Image
        src="/favicon.svg" // Remember to add your logo file to /public
        width={100}
        height={100}
        alt="Logo"
        className="w-6 md:w-8"
      />
      <span className="sr-only">About Monit</span>
    </Button>
  );
  ...
  const handleContactMe = () => {
    setIsOpen(false);
    // Force a full reload of the page with the query
            ...
            <DialogHeader className="relative flex flex-row items-start justify-between px-8 pt-8 pb-6">
              <div>
                <DialogTitle className="flex items-center gap-2 text-4xl font-bold tracking-tight">
                  Welcome to Monit's AI Portfolio
                </DialogTitle>
